...
class database:
    ...
    def update(self, row):
        self._db.execute('update {} set i1 = ? where t1 = ?'.format(self._table), (row['t1'], row['i1']))
        self._db.commit()
    ...
    def disp_rows(self):
        cursor = self._db.execute('select * from {} where t1 = ?'.format(self._table))
        for row in cursor:
            yield dict(row)
...
def main():
    ...
    db.sql_do('create table test ( t1 test, i1 int )')
